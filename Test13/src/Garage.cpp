#include "Garage.h"
Garage Garage::instance; // statique
...
void Garage::deleteContractById(int id)
{
	...
	while(it != contracts.end() && it->getId() != id)
	{
		it++;
	}
...
Contract Garage::findContractById(int id) const
{
	...
	while (it != contracts.cend() && it->getId() != id)
	{
		it++;
	}
...
int Garage::load()
{
	...
	NomConcat = "contract.xml";
	XmlFileSerializer<Contract> *FCO = nullptr;
	try
	{
		FCO = new XmlFileSerializer<Contract>(NomConcat, XmlFileSerializer<Contract>::READ);
	}
	catch(const XmlFileSerializerException& x)
	{
		cout << "Erreur: " << x.getMessage() << endl;
